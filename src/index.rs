...
use crate::storage::{
    ArrowStorage, IndexBinaryStorage, IndexMetadata, SoAArrowStorage, SoAIndexBinaryStorage,
    SoASplitIndexBinaryStorage, SplitIndexBinaryStorage,
};

use crate::interval::Interval;
use crate::sort::{
    IndexBin, IndexSortable, MassType, ParentSortedIndexBinSearchIter, SoAIndexBin,
    SoAIndexSortable, SoAParentSortedIndexBinSearchIter, SortType, Tolerance,
};
...
macro_rules! impl_common_search {
    () => {
...
{
    pub bins: Vec<SoAIndexBin<T, TV>>,
    pub parents: SoAIndexBin<P, PV>,
    pub(crate) bins_per_dalton: u32,
    pub(crate) max_item_mass: MassType,
    pub(crate) sort_type: SortType,
}
...
mod soa_storage {
        ...
        for<'t> PV::Ref<'t>: IndexSortable,
    {
        ...
        fn from_components(
            metadata: IndexMetadata,
            parents: PV,
            entries: HashMap<u64, TV>,
        ) -> Self {
            let mut parents = SoAIndexBin::from(parents);
            parents.assume_sorted(SortType::ByMass);
            let mut this = Self::empty(metadata.bins_per_dalton, metadata.max_item_mass);
            this.parents = parents;
            entries.into_iter().for_each(|(k, b)| {
                let mut bin = SoAIndexBin::from(b);
                bin.assume_sorted(SortType::ByParentId);
                this.bins[k as usize] = bin;
            });
            this
...
{
    pub fn new(
        bins: Vec<SoAIndexBin<T, TV>>,
        parents: SoAIndexBin<P, PV>,
        bins_per_dalton: u32,
        max_item_mass: MassType,
        sort_type: SortType,
    ) -> Self {
    ...
    pub fn iter_bins(&self) -> std::slice::Iter<'_, SoAIndexBin<T, TV>> {
        self.bins.iter()
    }
    ...
    for<'t> PV::Ref<'t>: IndexSortable,
{
    pub fn new(
        ...
        let (low_mass, high_mass) = error_tolerance.bounds(query);
        let low_bin = index.bin_for_mass(low_mass);
        let high_bin: usize = index.bin_for_mass(high_mass) + 1;
        let n_bins: usize = index.bins.len();
        let high_bin = if high_bin > n_bins { n_bins } else { high_bin };
        Self {
            index,
            query,
            error_tolerance,
    ...
    #[inline(always)]
    fn next_bin(&mut self) -> Option<&'a SoAIndexBin<T, TV>> {
        if self.current_bin < self.high_bin {
            let bin = unsafe { self.index.bins.get_unchecked(self.current_bin) };
            self.current_bin += 1;
            Some(bin)
    ...
    for<'t> PV::Ref<'t>: IndexSortable,
{
    pub fn new(
        ...
        let bin_iter = SoASearchIndexBinIter::new(index, query, error_tolerance);
        let mut this = Self {
            query,
            error_tolerance,
            parent_range,
    ...
    fn next_bin_iterator(&mut self) -> bool {
        self.item_iter = self.bin_iter.next().map(|bin| {
            SoAParentSortedIndexBinSearchIter::new(
                bin,
                self.parent_range,
                self.query,
                self.error_tolerance,
    ...
    #[inline(always)]
    fn next_entry(&mut self) -> Option<TV::Ref<'a>> {
        loop {
            if let Some(t) = self.item_iter.as_mut().and_then(|it| it.next()) {
                return Some(t);
            } else {
...
mod test {
    ...
    fn test_build_soa() {
        ...
        let mut parent_list: SoAIndexBin<Spectrum, SpectrumVec> =
            SoAIndexBin::new(spectra.into_iter().collect(), SortType::Unsorted, 0.0, 0.0);
        parent_list.sort(SortType::ByMass);
