...
use mass_fragment_index::{
    sort::SortType, DeconvolutedPeak, Spectrum, index::SoASearchIndex
};
...
fn main() -> io::Result<()> {
    ...
    let mut index = SoASearchIndex::empty(100, 3000.0);
    ...
    index.write_banded_parquet(&storage_dir, Default::default(), None)?;
    ...
    for (p1, p2) in index.parents.iter().zip(duplicate.parents.iter()) {
        assert_eq!(p1.scan_number, p2.scan_number);
    }
